  ...
  <br>
    <div id="container" class="container">
      ...
      <div id="quiz-container" style="display: none;">
        ...
        <!-- Timer -->
        <p class="p">Waktu tersisa: <span id="time-left">15</span> detik </p>
      </div>
      <!-- Hasil -->
      <div id="result-container" style="display: none;">
        ...
        <button id="simpan">Simpan Rank</button>
    ...
    <script>
      ...
      let score = 0;
      let timeTaken = 0; // Waktu yang dihabiskan dalam detik
...
}// Adding debugging logs to the event listener and AJAX
...
function saveScoreAndShowLeaderboard() {
    ...
    const percentage = ((score1 / questions.length) * 100);
...
xhr.send(`username=${username}&score=${score}&total_questions=${totalQuestions}&time_taken=${timeTaken}`);
}
...
function showLeaderboard() {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            document.getElementById('leaderboard-container').innerHTML = xhr.responseText;
        }
    };
    xhr.send();
}
...
function startTimer() {
    let timeLeft = 15; // Set waktu awal (15 detik)
    ...
    timer = setInterval(() => {
        ...
        // Menambahkan waktu yang telah berlalu ke timeTaken
        timeTaken++;
...
function endQuiz() {
    ...
    const score1 = Math.round((score / questions.length) * 100);
    
    // Tampilkan container hasil dan sembunyikan container kuis
    document.getElementById('quiz-container').style.display = 'none';
    document.getElementById('result-container').style.display = 'block';

    // Update informasi hasil
    ...
    document.getElementById('result-message').innerHTML = `Waktu yang digunakan: ${timeTaken} detik`;
...
// Query to fetch leaderboard data with percentage and time
...
if (mysqli_num_rows($result) > 0) {
    ...
    // Wrap the table in a responsive container (using Bootstrap class)
    echo "<div class='table-responsive'>";
    echo "<table class='table table-bordered'>";
    echo "<thead><tr><th>Rank</th><th>Username</th><th>Soal</th><th>Percentage</th><th>Time Taken</th><th>Date</th></tr></thead>";
    echo "<tbody>";

    $rank = 1;
    while ($row = mysqli_fetch_assoc($result)) {
        // Format time (if stored in seconds, convert to mm:ss format)
        $time_taken = gmdate("i:s", $row['time_taken']); // Assuming time_taken is in seconds
        
        // Format percentage, ensure it's shown correctly
        $percentage = isset($row['percentage']) ? $row['percentage'] . "%" : "N/A";
