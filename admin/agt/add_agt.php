...
$carikode = mysqli_query(
    $koneksi,
    "SELECT user_id FROM tb_anggota ORDER BY user_id DESC"
);
$datakode = mysqli_fetch_array($carikode);
// Check if there's a previous ID, otherwise set a default value
if ($datakode) {
    // Extract the numeric part from the previous ID (assuming ID format is "A001", "A002", etc.)
    $kode = $datakode["user_id"];
    $urut = substr($kode, 1, 3); // Extract the number part
    $tambah = (int) $urut + 1; // Increment the number
} else {
    // If there's no previous ID, start from 1
    $tambah = 1;
}
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="user_id" id="user_id" class="form-control" value="<?php echo $format; ?>"
                             readonly/>
                        </div>
